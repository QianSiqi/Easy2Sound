...
class Easy2SoundApp {
    constructor() {
        ...
        // 当前活跃工具按钮组（互斥）
        this.toolButtons = ['btn-pencil', 'btn-select', 'btn-erase', 'btn-pitch'];
    ...
    _bindToolbar() {
        ...
        $('btn-pitch')?.addEventListener('click', () => this._setTool('pitch'));
    ...
    _bindMenus() {
        ...
        const menuDefs = {
            ...
            options: [
                ...
                { label: '音高编辑', shortcut: 'V', action: () => this._setTool('pitch') },
            ],
            help: [
    ...
    _bindKeyboard() {
        document.addEventListener('keydown', (e) => {
            ...
            else if (e.key === 'v' && !ctrl) { this._setTool('pitch'); }
        });
    }
        ...
        switch (action) {
            ...
            case 'insert-note':
                ...
                const newNote = {
                    ...
                    vib_start: '', vib_end: '', vib_hz: '', vib_hard: '',
                    pt_x: '', pt_y: ''
                };
    ...
    async saveProject() {
        try {
            ...
            let filePath = '';
            try {
                const dlgRes = await fetch('/api/files/dialog', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ mode: 'save', file_types: ['E2S 文件 (*.e2s)'] }),
                });
                const dlgData = await dlgRes.json();
                if (dlgData.ok && dlgData.path) {
                    filePath = dlgData.path;
                } else {
                    // 对话框不可用，使用默认路径
                    filePath = '';
                }
            } catch (e) {
                console.warn('File dialog failed, using default path:', e);
            }

            // 如果没选路径，用默认路径
            if (!filePath) {
                filePath = `tmp/${this.projectName}.e2s`;
            }

            // 确保后缀为 .e2s
            if (!filePath.toLowerCase().endsWith('.e2s')) {
                filePath += '.e2s';
            }
            ...
            if (result.ok) {
                this._updateStatusBar(`已保存: ${result.path || filePath}`);
            }
        } catch (err) {
    ...
    async runPhonemer(trackId) {
        ...
        try {
            ...
            if (trackId === this.currentTrackId && updatedTrack) {
                ...
                // 刷新选中音符的属性面板
                const selectedNotes = this.pianoRoll.getSelectedNotes();
                if (selectedNotes.length > 0) {
                    const selectedNote = updatedTrack.notes.find(n => n.id === selectedNotes[0].id);
                    if (selectedNote) {
